...
import { getList, getListStatus, fetchList } from "../supplierSlice";
...
const SuppliersListView = () => {
    ...
    switch(listStatus) {
        case 'loading':
            ...
            break;
        case 'succeeded':
            content = list.map(item => <tr>
                <td className="px-2 py-4">{item.name}</td>
                <td><button className="border border-red-500 rounded-sm bg-white text-red-500 p-0.5 mx-0.5" 
                alt="Delete">
            ...
            break;
        case 'failed':
            content = <p>Error loading content for suppliers</p>;
    } 

    return (
        <section>
            <div className="block bg-gray-100 border-gray-300 border-1 rounded-md border p-3 shadow-sm max-w-full mb-2">
                <h3 className="text-lg text-amber-900">Suppliers</h3>
                <table className="w-full text-sm text-left text-gray-500 my-1 table-auto overflow-scroll">        
